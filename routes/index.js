...
router.post('/settings', function(req, res, next) {
  req.pool.getConnection(function(err, connection) {
    ...
    // Start query
    var query = "UPDATE account SET email=?, password_hash=SHA2(?, 256) WHERE account_id='"+req.session.userid+"'";
    connection.query(query, [req.body.email, req.body.pass], function(err, result, fields) {
      connection.release();  // release connection
      res.send();
    })
  });
